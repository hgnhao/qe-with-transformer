...
class VSM:
    ...
    def get_document_vector(self, doc_id: int, scheme: str, tf_variant: str = "raw") -> Dict[str, float]:
        """
        Calculates the weight vector for a given document.
        scheme: 'tf', 'idf', 'tf-idf', 'tf-idf-cosine'
        """
        vector = {}
        max_tf = self.ir.doc_max_freqs.get(doc_id, 1)
        N = self.ir.document_count
        
        # We only need to iterate over terms present in the document
        # But inverted index gives us doc_ids for a term, we don't have a direct doc -> terms map.
        # We can extract it from inverted index or preprocess it.
        # It's more efficient to just iterate over the query terms when calculating similarity,
        # but to get the document vector norm, we need all terms in the doc.
        
        return vector # We usually don't build full doc vectors due to sparsity, we calculate similarity directly.
    ...
    def calculate_similarity(self, query_terms: Dict[str, float], scheme: str, tf_variant: str = "raw") -> Dict[int, float]:
        ...
        query_terms: dict of term -> weight (e.g. from expanded query)
        """
        ...
        # We pre-calculate document norms if cosine normalization is requested
        if scheme == "tf-idf-cosine":
            # This is expensive to do on the fly, ideally should be cached when index is built.
            # But for CISI (1460 docs), we can do it, or we calculate it efficiently.
            pass
            
        # To avoid calculating full doc norms on the fly every time, let's just 
